LIMIT_SWITCH_GPIO_CONFIGURATION = [
    {'name': 'GPIO2_X_LIMIT_SWITCH_HOME','type': 'pi', 'physical': '3',  'wPi': '8', 'BCM': '2',  'mode': 'in',   'value': '0'},
    {'name': 'GPIO3_X_LIMIT_SWITCH_END', 'type': 'pi', 'physical': '5',  'wPi': '9', 'BCM': '3',  'mode': 'in',   'value': '0'},
    {'name': 'GPIO4_LIMIT_SWITCH_Y_1',  'type': 'pi', 'physical': '7',  'wPi': '7', 'BCM': '4',  'mode': 'in',   'value': '0'},
    {'name': 'GPIO14_LIMIT_SWITCH_Y_2', 'type': 'pi', 'physical': '8',  'wPi': '15','BCM': '14', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO15_LIMIT_SWITCH_Z_1', 'type': 'pi', 'physical': '10', 'wPi': '16','BCM': '15', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO17_LIMIT_SWITCH_Z_2', 'type': 'pi', 'physical': '11', 'wPi': '0', 'BCM': '17', 'mode': 'in',   'value': '0'},
]
MOTOR_GPIO_CONFIGURATION = [
    {'name': 'GPIO18_X_MOTOR_DIRECTION','type': 'pi', 'physical': '12', 'wPi': '1', 'BCM': '18', 'mode': 'alt0',  'value': '0'},
    {'name': 'GPIO27_X_MOTOR_PWM',      'type': 'pi', 'physical': '13', 'wPi': '2', 'BCM': '27', 'mode': 'out',   'value': '0'},
    {'name': 'GPIO22_X_MOTOR_MS1',      'type': 'pi', 'physical': '15', 'wPi': '3', 'BCM': '22', 'mode': 'out',   'value': '0'},
    {'name': 'GPIO23_X_MOTOR_MS2',      'type': 'pi', 'physical': '16', 'wPi': '4', 'BCM': '23', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO24_X_MOTOR_MS3',      'type': 'pi', 'physical': '18', 'wPi': '5', 'BCM': '24', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO10_Y_MOTOR_DIRECTION','type': 'pi', 'physical': '19', 'wPi': '12','BCM': '10', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO9_Y_MOTOR_PWM',       'type': 'pi', 'physical': '21', 'wPi': '13','BCM': '9',  'mode': 'in',   'value': '0'},
    {'name': 'GPIO25_Y_MOTOR_MS1',      'type': 'pi', 'physical': '22', 'wPi': '6', 'BCM': '25', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO11_Y_MOTOR_MS2',      'type': 'pi', 'physical': '23', 'wPi': '14','BCM': '11', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO8_Y_MOTOR_MS3',       'type': 'pi', 'physical': '24', 'wPi': '10','BCM': '8',  'mode': 'in',   'value': '0'},
    {'name': 'GPIO7_Z_MOTOR_DIRECTION', 'type': 'pi', 'physical': '26', 'wPi': '11','BCM': '7',  'mode': 'in',   'value': '0'},
    {'name': 'GPIO0_Z_MOTOR_PWM',       'type': 'pi', 'physical': '27', 'wPi': '30','BCM': '0',  'mode': 'alt0', 'value': '0'},
    {'name': 'GPIO1_Z_MOTOR_MS1',       'type': 'pi', 'physical': '28', 'wPi': '31','BCM': '1',  'mode': 'alt0', 'value': '0'},
    {'name': 'GPIO5_Z_MOTOR_MS2',       'type': 'pi', 'physical': '29', 'wPi': '21','BCM': '5',  'mode': 'in',   'value': '0'},
    {'name': 'GPIO6_Z_MOTOR_MS2',       'type': 'pi', 'physical': '31', 'wPi': '22','BCM': '6',  'mode': 'in',   'value': '0'},
]
RPI_GPIO_NO_USED_CONFIGURATION = [ 
    {'name': 'GPIO12_NO_USE',           'type': 'pi', 'physical': '32', 'wPi': '26','BCM': '12', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO13_NO_USE',           'type': 'pi', 'physical': '33', 'wPi': '23','BCM': '13', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO19_AUDIO_PCM_FS',     'type': 'pi', 'physical': '35', 'wPi': '24','BCM': '19', 'mode': 'alt0',   'value': '0'},
    {'name': 'GPIO16_NO_USE',           'type': 'pi', 'physical': '36', 'wPi': '27','BCM': '16', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO26_NO_USE',           'type': 'pi', 'physical': '37', 'wPi': '25','BCM': '26', 'mode': 'in',   'value': '0'},
    {'name': 'GPIO20_AUDIO_PCM_DIN',    'type': 'pi', 'physical': '38', 'wPi': '28','BCM': '20', 'mode': 'alt0',   'value': '0'},
    {'name': 'GPIO21_AUDIO_PCM_DOUT',   'type': 'pi', 'physical': '40', 'wPi': '29','BCM': '21', 'mode': 'in',   'value': '0'},
]